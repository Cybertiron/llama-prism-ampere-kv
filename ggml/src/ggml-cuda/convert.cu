...
// q2_K non-contiguous dequant (KV cache view): mirrors the q3_K nc path
template<typename dst_t>
static __global__ void dequantize_block_q2_K_nc(
        const void * __restrict__ vx, dst_t * __restrict__ y,
        const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t ne0203,
        const int64_t s01, const int64_t s02, const int64_t s03) {
    const int64_t nblk_row = ne00 / QK_K;
    ...
    for (int64_t i0203 = blockIdx.z; i0203 < ne0203; i0203 += gridDim.z) {
        ...
        const int64_t i03 = i0203 / ne02;
        for (int64_t i01 = blockIdx.y; i01 < ne01; i01 += gridDim.y) {
            const int64_t src_ib  = i03*s03 + i02*s02 + i01*s01 + cb;
            const int64_t dst_off = (i0203*ne01 + i01)*ne00 + cb*QK_K;
            dequantize_q2_K(vx, src_ib, y + dst_off, threadIdx.x);
        }
    }
}

template<typename dst_t>
static void dequantize_row_q2_K_nc_cuda(
        const void * vx, dst_t * y,
        const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t ne03,
        const int64_t s01, const int64_t s02, const int64_t s03, cudaStream_t stream) {
    const int64_t ne0203 = ne02*ne03;
    ...
    const dim3 grid((unsigned) nblk_row,
                    (unsigned) std::min(ne01,   (int64_t) 65535),
                    (unsigned) std::min(ne0203, (int64_t) 65535));
    dequantize_block_q2_K_nc<<<grid, 64, 0, stream>>>(vx, y, ne00, ne01, ne02, ne0203, s01, s02, s03);
}
...
to_fp16_nc_cuda_t ggml_get_to_fp16_nc_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q2_K:
            return dequantize_row_q2_K_nc_cuda;
        case GGML_TYPE_BF16:
